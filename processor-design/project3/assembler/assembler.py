...
class Assembler:
				...
				if imm in self.labels:
					...
					if (opCode in self.primary and self.primary[opCode][2]) or (opCode in self.pseudo and self.pseudo[opCode][1][2]):
						...
						if opCode == "JAL" or opCode == "CALL" or opCode == "JMP":
							#print opCode
							outputFile.write(format(each[1],'08x')+" : "+ format(int(each[0]+self.binary((self.labels[imm] - 1)*4,immSize,immSize),2),'08x')+";\n")
						#for branches
						else:
							outputFile.write(format(each[1],'08x')+" : "+ format(int(each[0]+self.binary((self.labels[imm]-(each[1]+1))*4,immSize,immSize),2),'08x')+";\n")
					else:
						if each[3] == "SUBI":
							outputFile.write(format(each[1],'08x')+" : "+ format(int(each[0]+ self.negateBin(self.binary((self.labels[imm]-1)*4,immSize,immSize)),2),'08x')+";\n")
						else:
							outputFile.write(format(each[1],'08x')+" : "+ format(int(each[0]+ self.binary((self.labels[imm]-1) *4 ,immSize,immSize),2), '08x')+";\n")
	...
	def translateInstruction(self,instr):
		...
		if (org1 in instr or org2 in instr) and name not in instr and word not in instr and label not in instr:
			...
			if self.orig != currLoc:
				self.instructions.append(["["+format(self.orig, '08x')+".."+format(newLoc, '08x')+"] : 0000;",self.lnumber,""])
	...
	def makeInstructionPseudo(self,code,instrType,opCode, opCodeInstr, instr):
		...
		if opCode == "NOT":
			...
			self.instructions.append([opCodeInstr+Rx+Ry+imm,self.lnumber,instr.strip() + "=>" + self.pseudo[opCode][0]+" "+RxNum+", "+RyNum+", "+"1\n",opCode,imm])
		#CALL Imm(Ri) => JAL RA,Imm(Ri) and JMP Imm(Ri) => JAL R6,Imm(Ri)
			...
			if opCode == "CALL":
				...
				self.instructions.append([opCodeInstr+Rx+Ry,self.lnumber,instr.strip()+"=>"+self.pseudo[opCode][0]+" RA, "+imm+"("+Ry+")\n",opCode,imm])
			elif opCode == "JMP":
				Rx = self.binary(self.registers["R6"],self.REGISTER_LENGTH,self.REGISTER_LENGTH)
				self.instructions.append([opCodeInstr+Rx+Ry,self.lnumber,instr.strip()+"=>"+self.pseudo[opCode][0]+" R6, "+imm+"("+Ry+")\n",opCode,imm])
		#RET => JAL R6,0(RA)
		elif opCode == "RET":
			Rx = self.binary(self.registers["R6"],self.REGISTER_LENGTH,self.REGISTER_LENGTH)
			imm = "0"*self.IMM_LENGTH
			Ry = self.binary(self.registers["RA"],self.REGISTER_LENGTH,self.REGISTER_LENGTH)
			self.instructions.append([opCodeInstr+Rx+Ry+imm,self.lnumber,instr.strip()+"=>"+self.pseudo[opCode][0]+" R6, "+imm+"(RA)\n",opCode,imm])
		...
		#GT Rx,Rz,Ry => LT Rx,Ry,Rz and GE Rx,Rz,Ry => LE Rx,Ry,Rz
			...
			self.instructions.append([self.primary["EXT"][0]+Rx+Ry+Rz+"0000000"+opCodeInstr, self.lnumber,instr.strip()+"=>"+self.pseudo[opCode][0]+" "+RxNum+", "+RyNum+", "+RzNum+"\n",opCode])
	...
	def makeInstruction(self, code, instrType, opCode, opCodeInstr, instr):
		...
		if instrType == "J":
			...
			self.instructions.append([opCodeInstr+Rx+Ry,self.lnumber,instr,opCode,code[1].strip()])
		...
		#for R-type: Rx, Ry, Rz
			...
			self.instructions.append([self.primary["EXT"][0]+Rx+Ry+Rz+"0000000"+opCodeInstr, self.lnumber,instr,opCode])
