...
class Assembler:
	...
	def __init__(self, args):
		...
		self.pseudo = {
		"NOT": self.primary["XORI"],
		"CALL": self.primary["JAL"],
		"RET": self.primary["JAL"],
		"JMP": self.primary["JAL"],
		"BGT": self.primary["BLT"],
		"BGE": self.primary["BLE"],
		"GT": self.secondary["LT"],
		"GE": self.secondary["LE"],
		"SUBI": self.primary["ADDI"],
		}
		...
		for each in self.instructions:
			if "DEAD" in each[0]:
				...
				if imm in self.labels:
					...
					#for branches
					if opCode in self.primary and self.primary[opCode][2]:
					...
					"""if each[1] == 12:
						#imm = self.checkHex(imm).zfill(immSize)
						#length = len(imm)
						#print imm
						print self.checkHex(imm)
						#imm = '001'
						#print self.checkHex('001') == self.checkHex('111')
						#immediate = self.checkImmLength(self.checkHex(imm), immSize, each[2])
						print each[2]+ ":" + str(immediate)
					if each[1] == 30:
						#print imm
						print self.checkHex(imm)
						print each[2]+ ":" + str(immediate)"""
					if each[3] == "SUBI":
						...
						outputFile.write(format(each[1],'08x')+" : "+ format(int(each[0]+ immediate,2), '08x')+";\n")
		outputFile.write("["+format(self.lnumber,'08x')+"..07ff] : DEAD;\nEND;")
		outputFile.close()
	...
	def translateInstruction(self,instr):
		"""translate a single instruction into machine code"""

		# split instr into pieces on ',' and strip whitespace 
		"""instrList = instr.split(str=',')
		instrList = [str.strip() for str in instrList]

		# open json to translate instructions
		jsonData = open(ISA_JSON_FILE)
		instrToOpcode = json.load(jsonData)

		# translate instruction name into opcode
		opcode = instrToOpcode[instrList[0]];
		-format(255, '08x'))
		print opcode"""
		org1 = ".ORIG"
		org2 = ".ORG"
		...
		if (org1 in instr or org2 in instr) and name not in instr and word not in instr and label not in instr:
			...
			if self.orig != currLoc:
				self.instructions.append(["["+format(self.orig, '08x')+".."+format(newLoc, '08x')+"] : DEAD;",self.lnumber,""])
		...
		#This takes care of .NAME statements 
			...
			line = line.split("<>")
			opCode = line[0].upper()
			if opCode in self.primary:
				self.makeInstruction(line[1], self.primary[opCode][1], opCode, self.primary[opCode][0], instr)
				self.lnumber += 1
				self.anumber += 4
			elif opCode in self.secondary:
				self.makeInstruction(line[1], self.secondary[opCode][1], opCode, self.secondary[opCode][0], instr)
				self.lnumber += 1
				self.anumber += 4
			elif opCode in self.pseudo:
				self.makeInstructionPseudo(line[1], self.pseudo[opCode][1], opCode, self.pseudo[opCode][0], instr)
				self.lnumber += 1
				self.anumber += 4
			#rest = line[1]
			#print opCode
	...
	def makeInstructionPseudo(self,code,instrType,opCode, opCodeInstr, instr):
		...
		if opCode == "NOT":
			...
			self.instructions.append([opCodeInstr+Ry+Rx+imm,self.lnumber,instr,opCode,imm])
		#CALL Imm(Ri) => JAL RA,Imm(Ri) and JMP Imm(Ri) => JAL R6,Imm(Ri)
			...
			self.instructions.append([opCodeInstr+Ry+Rx,self.lnumber,instr,opCode,imm])
		#RET => JAL R6,0(RA)
		elif opCode == "RET":
			Rx = self.binary(self.registers["R6"],self.REGISTER_LENGTH,self.REGISTER_LENGTH)
		...
		#BGT Ry,Rx,Label => BLT Rx,Ry,Label and BGE Ry,Rx,Label => BLE Rx,Ry,Label
			...
			code = code.strip().split(",")
			Rx = self.binary(self.registers[code[1].strip().upper()],self.REGISTER_LENGTH,self.REGISTER_LENGTH)
			Ry = self.binary(self.registers[code[0].strip().upper()],self.REGISTER_LENGTH,self.REGISTER_LENGTH)
			self.instructions.append([opCodeInstr+Ry+Rx,self.lnumber,instr,opCode,code[2].strip()])
		#GT Rx,Ry,Rz => LT Rx,Rz,Ry and GE Rx,Ry,Rz => LE Rx,Rz,Ry
		elif opCode == "GT" or opCode == "GE":
			code = code.strip().split(",")
			Rx = self.binary(self.registers[code[0].strip().upper()],self.REGISTER_LENGTH,self.REGISTER_LENGTH)
			Ry = self.binary(self.registers[code[2].strip().upper()],self.REGISTER_LENGTH,self.REGISTER_LENGTH)
			Rz = self.binary(self.registers[code[1].strip().upper()],self.REGISTER_LENGTH,self.REGISTER_LENGTH)
	...
	def makeInstruction(self, code, instrType, opCode, opCodeInstr, instr):
